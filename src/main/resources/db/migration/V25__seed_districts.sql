...
BEGIN
    FOR r IN
        ...
        ) AS seed(province_code, code, name_km, name_en)
    LOOP
        SELECT p.id
        INTO resolved_province_id
        FROM provinces p
        WHERE p.code = r.province_code
        LIMIT 1;

        IF resolved_province_id IS NULL THEN
            RAISE EXCEPTION
                'Cannot seed district %: province code % was not found',
                r.code,
                r.province_code;
        END IF;
        ...
        IF target_id IS NULL THEN
            ...
            INTO target_id
            FROM districts d
            WHERE d.province_id = resolved_province_id
              AND (
                    d.name_km = r.name_km
                 OR LOWER(d.name_en) = LOWER(r.name_en)
              )
            ORDER BY d.id
            LIMIT 1;
        END IF;
